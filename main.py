...
#my variables and settings
Motor_Pin1 = 18
GPIO.setmode(GPIO.BOARD)
GPIO.setup(Motor_Pin1, GPIO.OUT)
p = GPIO.PWM(Motor_Pin1, 50) # GPIO 17 for PWM with 50Hz
eat_period = 10
cat_timer = 0
detected_cat = False
now = datetime.strptime("2018-01-31", "%Y-%m-%d")
remain_times = 5
...
def pet_detector(frame):
    ...
    if cat_timer > 10:
            detected_cat = True
            door_open()
            if now == datetime.strptime("2018-01-31", "%Y-%m-%d"):
                remain_times-=1
                print(remain_times)
            door_close()
